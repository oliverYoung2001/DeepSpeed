...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        self.losses = 0.0
        # if dist.get_rank():
        #     print(f'in DeepSpeedEngine !!!', flush=True)
        ...
        # if dist.get_rank() == 0:
        #     print(f'self.param_names: {self.param_names}')  # all 0, ???
        ...
        if optimizer or self.optimizer_name():  # True, optimizer is not None
            has_optimizer = True
        # If no parameters given by init default to module parameters
        if model_parameters is None:    # True
        ...
        # if dist.get_rank() == 0:
        #     print(f'model_parameters: {model_parameters}')  # all 0, why ???
        ...
        # print(f'hasattr(model, "pruners"): {hasattr(model, "pruners")}')    # False
        if hasattr(model, 'pruners'):   # False
        ...
        # print(f'LABEL4', flush=True)
        # print(f'isinstance(self.optimizer, DeepSpeedZeRoOffload): {isinstance(self.optimizer, DeepSpeedZeRoOffload)}')  # False
        if not isinstance(self.optimizer, DeepSpeedZeRoOffload):    # True
    ...
    def _broadcast_model(self):
        ...
        for p in self.module.parameters():
            ...
            # print(f'is_moe_param(p): {is_moe_param(p)}', flush=True)    # False
            if is_moe_param(p): # False
    ...
    def _set_client_model(self, model):
        ...
        self.__dict__['module'] = model     # [NOTE]: set self.module = model

    def _configure_distributed_model(self, model):
        self._set_client_model(model)
        # print(f'L1.0.1', flush=True)
        ...
        # print(f'self.has_moe_layers: {self.has_moe_layers}', flush=True)    # False
        ...
        # print(f'L1.0.6', flush=True)

        if not (self.amp_enabled() or is_zero_init_model):  # True
            self._broadcast_model()
        # print(f'L1.0.7', flush=True)
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        """[NOTE]: model_parameters are not used !!!

        Args:
            client_optimizer (_type_): _description_
            model_parameters (_type_): _description_

        Raises:
            ZeRORuntimeException: _description_

        Yields:
            _type_: _description_
        """
        if client_optimizer is None:    # False
            basic_optimizer = self._configure_basic_optimizer(model_parameters)
            log_dist(f"Using DeepSpeed Optimizer param name {self.optimizer_name()} as basic optimizer", ranks=[0])
        else:
            # if dist.get_rank() == 0:
            #     print(f'isinstance(client_optimizer, tuple(self._supported_optims())): {isinstance(client_optimizer, tuple(self._supported_optims()))}')    # True
            if isinstance(client_optimizer, tuple(self._supported_optims())):   # True
        ...
        # if dist.get_rank() == 0:
        #     print(f'basic_optimizer.param_groups: {basic_optimizer.param_groups}')  # all 0, why ???
    ...
    def _configure_zero_optimizer(self, optimizer):
        ...
        # print(f'model_dtype: {model_dtype}, gradient_accumulation_dtype: {gradient_accumulation_dtype}, mics_shard_size: {mics_shard_size}')    # fp16, fp16, - 1
        ...
        if zero_stage <= ZeroStageEnum.gradients:
            ...
            if isinstance(optimizer, DummyOptim):   # False
    ...
    def forward(self, *inputs, **kwargs):
        ...
        if self.zero_optimization_partition_weights():
            ...
            # if dist.get_rank() == 0:
            #     print(f'All modules:')  # all nested modules
            for module in self.module.modules():
                # if dist.get_rank() == 0:
                #     print(f'{module}')
                #     # print(f'{module._parameters}')
        ...
        loss = self.module(*inputs, **kwargs)   # [NOTE]: original forward
    ...
    def allreduce_gradients(self, bucket_size=MEMORY_OPT_ALLREDUCE_SIZE):
        # if dist.get_rank():
        #     print(f'in allreduce_gradients with stage{2 if self.zero_optimization_partition_gradients() else 1}', flush=True)
        ...
        if self.zero_optimization_partition_gradients():    # Reachable in stage2
            self.optimizer.overlapping_partition_gradients_reduce_epilogue()    # [NOTE]: have a look !!!
    ...
    @instrument_w_nvtx
    def backward_old(self, loss, allreduce_gradients=True, release_loss=False, retain_graph=False, scale_wrt_gas=True):
        ...
        """
        # if dist.get_rank() == 0:
        #     print(f'in engine.py/backward !!!', flush=True)
        ...
        if self.zero_optimization():    # True
            self.optimizer.is_gradient_accumulation_boundary = self.is_gradient_accumulation_boundary()
            self.optimizer.backward(loss, retain_graph=retain_graph)    # [NOTE]: important !!! (but no comm in stage1/2)
        ...
        # print(f'allreduce_gradients: {allreduce_gradients}')    # True
        if allreduce_gradients and self.enable_backward_allreduce:  # True, True
    ...
    def backward(self, loss, allreduce_gradients=True, release_loss=False, retain_graph=False, scale_wrt_gas=True):
        ...
        # if self.gradient_accumulation_steps() > 1 and scale_wrt_gas:
        #     loss = self._scale_loss_by_gas(loss.float())
        # print(f'loss: {loss.shape}', flush=True)
        # Log training loss
        self.losses += loss.mean().item()
        assert self.optimizer is not None and not isinstance(self.optimizer, DummyOptim), \
            "must provide optimizer during init in order to use backward"
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            # if dist.get_rank() == 0:
                # print(f'self.eigenvalue_enabled(): {self.eigenvalue_enabled()}')    # False
                # print(f'{self.eigenvalue_enabled() and not self.gas_boundary_ctr % self.eigenvalue_gas_boundary_resolution() and self.quantizer.any_precision_switch()}') # False
            if (self.eigenvalue_enabled() and not self.gas_boundary_ctr % self.eigenvalue_gas_boundary_resolution() \
                    and self.quantizer.any_precision_switch()): # False
        ...
        # Log learning rate
        # if dist.get_rank():
        #     print(f'self.monitor.enabled: {self.monitor.enabled}', flush=True)  # False
        if self.monitor.enabled:    # False
            if self.is_gradient_accumulation_boundary():
                if self.global_rank == 0:
        ...
        # if dist.get_rank():
        #     print(f'flops_profiler_active: {flops_profiler_active}', flush=True)    # False
        if flops_profiler_active:   # False
    ...
    def _load_checkpoint(self,  # Unreachable
                         load_dir,
                         tag,
                         load_module_strict=True,
                         load_optimizer_states=True,
    ...
    def _save_checkpoint(self, save_dir, tag, client_state={}, exclude_frozen_parameters=False):    # Unreachable
