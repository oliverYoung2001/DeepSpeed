...
class InsertPostInitMethodToModuleSubClasses(object):
    ...
    def __init__(self, enabled=True, mem_efficient_linear=True, ds_config=None, dtype=None):
        ...
        if ds_config is not None and ds_config.weight_quantization_config and ds_config.weight_quantization_config.quantized_initialization: # ds_config.weight_quantization_config == None
            self.quantized_initialization = ds_config.weight_quantization_config.quantized_initialization

    def __enter__(self):
        # print(f'self.enabled: {self.enabled}')  # True if zero_stage == 3 else False
        if not self.enabled:
            return

        global zero_init_context
        if zero_init_context == 0:  # True for first time
    ...
    def patch_init_and_builtins(self):
        ...
        def partition_after(f):
            ...
            def wrapper(module, *args, **kwargs):
                ...
                # print_rank_0(f'before {type(module).__name__}: is_child_module: {is_child_module}', force=True)    # sometimes True, sometimes False
                f(module, *args, **kwargs)
                # print_rank_0(f'after {type(module).__name__}: is_child_module: {is_child_module}', force=True)    # sometimes True, sometimes False
                if is_child_module:
        ...
        def _init_subclass(cls, **kwargs):
            # print_rank_0(f'exec _init_subclass on {cls.__name__} !!!', force=True)  # Unreachable, which indicates that no subclass is created
            cls._old_init = cls.__init__
            cls.__init__ = partition_after(cls.__init__)

        # Replace .__init__() for all existing subclasses of torch.nn.Module recursively
        # print_rank_0(f'subclass:', debug=True)
        for subclass in get_all_subclasses(torch.nn.modules.module.Module):
            # print_rank_0(f'{subclass}', debug=True)   # all modules recursively
        ...
        # print_rank_0(f'Init.override_module_apply: {Init.override_module_apply}', force=True)   # True
        if Init.override_module_apply:  # True
        ...
        if self.mem_efficient_linear:   # True
            print_rank_0(
...
class Init(InsertPostInitMethodToModuleSubClasses):
    ...
    override_module_apply = get_config_default(DeepSpeedZeroConfig, "override_module_apply")    # True
    ...
    def __init__(
        ...
        # print_rank_0(f'module: {module}', force=True)   # None
        if module is not None:  # False
            assert isinstance(module, torch.nn.Module)
            # print(f'reachable !!!') # Not Reachable
    ...
    def _zero_init_param(self, param):  # [NOTE]: Important function !!!
        self._convert_to_deepspeed_param(param) # [NOTE]: Register some communication function and ds configs for param
        ...
        param.partition()       # [NOTE]: partition param to each rank (per-param-grained !!!)
    ...
    def _convert_to_zero_parameters(self, param_list):
        for param in param_list:
            ...
            # print_rank_0(f'before: {param.shape}', force=True)  # Not Reachable
    ...
    def _post_init_method(self, module):
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            # print_rank_0(f'{name}: is_zero_param(param): {is_zero_param(param)}', force=True)   # False
            if not is_zero_param(param):    # True
                ...
                # print_rank_0(f'before: {param.shape}', force=True)
                self._zero_init_param(param)
                # print_rank_0(f'after: {param.shape}', force=True)    # [NOTE]: why are they all *empty* params ?
                # print_rank_0(f'{param}: {param.ds_status}', force=True) # NOT_AVAILABLE
                print_rank_0(
    ...
    def _convert_to_deepspeed_param(self, param):
        ...
        print_rank_0(f'{param}: {param.ds_status}')
    ...
    def _all_gather(self, param_list, async_op=False, hierarchy=None):
        ...
        handles = []
        all_gather_list = []
        for param in param_list:    # [NOTE]: communicate at per-param-grained when async ???
            if param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
                if async_op:
        ...
        if not async_op:
            if len(all_gather_list) == 1:
                ...
                self._allgather_params_coalesced(all_gather_nonquantize_list, hierarchy, quantize=False)    # [NOTE]: coalesced communicate !!!
    ...
    def _partition(self, param_list, force=False, has_been_updated=False):
        for param in param_list:
            ...
            # print_rank_0(f'self.zero_param_process_group: {self.zero_param_process_group}', force=True) # None
            if self.zero_param_process_group is not None:   # False
    ...
    def _partition_param(self, param, buffer=None, has_been_updated=False):
        ...
        if param.ds_status is ZeroParamStatus.AVAILABLE:    # True
            ...
            if param.ds_tensor is not None and not has_been_updated:  ##param already partitioned   # False
                ...
                # print_rank_0(f'{param}', force=True)
                free_param(param)   # [NOTE]: free original param.data, sharded param are in param.ds_tensor
                # print_rank_0(f'{param}', force=True)
            ...
            # print_rank_0(f'tensor_size: {tensor_size}, partition_size: {partition_size}', force=True)   # param/tensor-level partition
            # print_rank_0(f'param.ds_tensor: {param.ds_tensor}', force=True) # None
            if param.ds_tensor is None: # True
                ...
                if self.remote_device == OffloadDeviceEnum.nvme and self.param_swapper.swappable_tensor(
                    ...
                    # print_rank_0(f'param.ds_persist: {param.ds_persist}', force=True)   # False
                    if param.ds_persist:    # False
                        device = self.local_device
                    elif self.remote_device == OffloadDeviceEnum.nvme:  # False
                        device = OffloadDeviceEnum.cpu
                    else:   # True
                        device = self.remote_device
                    # print_rank_0(f'device: {device}', force=True)   # cuda:0

                    partitioned_tensor = torch.empty(partition_size, dtype=param.dtype, device=device)
                    # quantize the tensor if it's not trainable
                    if not param.requires_grad and self.quantized_nontrainable_weights: # False
                        partitioned_tensor, partitioned_tensor.ds_quant_scale = self.quantizer_module.quantize(
                            partitioned_tensor)

                    if device == OffloadDeviceEnum.cpu and self.pin_memory: # False
            ...
            if start < param.ds_numel and end <= param.ds_numel:    # True if no padding
