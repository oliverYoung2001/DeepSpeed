...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        self.contiguous_gradients = contiguous_gradients or self.cpu_offload    # True
        ...
        # if dist.get_rank() == 0:
        #     print(f'mpu: {mpu}', flush=True)    # None
        if mpu is None: # True
        ...
        self.round_robin_gradients = round_robin_gradients  # False

        self.extra_large_param_to_reduce = None
        self.fp16_master_weights_and_gradients = fp16_master_weights_and_gradients

        if self.fp16_master_weights_and_gradients:  # False
        ...
        self.groups_padding = []
        # print(f'rank{dist.get_rank()}: in param_groups', flush=True)
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            for param in param_group['params']:
                # print(f'rank{dist.get_rank()}: {param.shape}, {param.dtype}, {param.device}', flush=True)
                # print(f'rank{dist.get_rank()}: {param.data}', flush=True)
            ...
            # if dist.get_rank() == 0:
            # print(f'self.bit16_groups[i]: {self.bit16_groups[i]}', flush=True)
            ...
            if not fp16_master_weights_and_gradients:   # True
    ...
    def reduce_gradients(self, pipeline_parallel=False):
        ...
        if pipeline_parallel and self.contiguous_gradients: # pipe
            self.ipg_buffer = []
    ...
    def average_tensor(self, tensor):   # [NOTE]: RS logic
        if self.overlap_comm:
    ...
    def buffered_reduce_fallback(self, rank, grads, elements_per_buffer=500000000, log=None):   # [NOTE]: RS logic
    ...
    def scaled_global_norm(self, norm_type=2):
        ...
        norm_groups = []
        # print(f'self.averaged_gradients: {self.averaged_gradients}', flush=True)
        for i, group in enumerate(self.bit16_groups):
            partition_id = dist.get_rank(group=self.real_dp_process_group[i])
            if self.cpu_offload:
    ...
    def step(self, closure=None):
        ...
        all_gather_dp_groups(groups_flat=self.bit16_groups_flat,    # [NOTE]: AG logic Here !!!
    ...
    def preprocess_backward(self):
        self.micro_step_id += 1
        # if dist.get_rank() == 0:
            # print(f'self.contiguous_gradients: {self.contiguous_gradients}', flush=True)    # True
        if self.contiguous_gradients:   # True
            self.ipg_buffer = []
            buf_0 = torch.empty(int(self.reduce_bucket_size),
                                dtype=self.dtype,
    ...
    def postprocess_backward(self):
        if self.use_grad_accum_attribute:   # False
    ...
    def backward(self, loss, retain_graph=False):
        ...
        """
        self.preprocess_backward()
        # if dist.get_rank() == 0:
        #     print(f'self.custom_loss_scaler: {self.custom_loss_scaler}', flush=True)    # False
        if self.custom_loss_scaler: # False
        ...
        # if dist.get_rank() == 0:
        #     print(f'self.use_grad_accum_attribute: {self.use_grad_accum_attribute}', flush=True)    # False
        self.postprocess_backward()
