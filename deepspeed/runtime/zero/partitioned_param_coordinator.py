...
class PartitionedParameterCoordinator:
    ...
    def record_parameters(self, sub_module: Module) -> None:
        ...
        # print_rank_0(f'step_id: {step_id}', force=True) # all 0
    ...
    def fetch_sub_module(self, current_submodule: Module, forward: bool) -> None:
        ...
        params_to_fetch = frozenset(iter_params(current_submodule)) # non-recursive
        fetch_numel = sum(
            [p.partition_numel() for p in params_to_fetch if p.ds_status == ZeroParamStatus.NOT_AVAILABLE])
        # print_rank_0(f'{type(current_submodule).__name__}: {fetch_numel}, forward: {forward}, step_id: {self.__step_id}', force=True)
        # for p in params_to_fetch:
        #     print_rank_0(f'{p.shape}, {p.ds_status}, {p.partition_numel()}', force=True)    # AVAILABLE when prefetch is executed
        if fetch_numel > 0:
            ...
            self.__all_gather_params(params_to_fetch, forward)  # [NOTE]: Allgather here !!!
        ...
        # print_rank_0(f'self.is_complete_trace(): {self.is_complete_trace()}, {self.__trace_mode}, {self.__step_id}', force=True)
        if self.is_complete_trace():    # Ture after first forward + backward
            ...
            if self.__prefetch_bucket_sz > 0:
                ...
                while self.__param_queue and numel_prefetching < max_params_to_prefetch:
                    ...
                    # print_rank_0(f'_is_currently_on_nvme(param_in_trace.param): {_is_currently_on_nvme(param_in_trace.param)}', force=True) # False
                    if _is_currently_on_nvme(param_in_trace.param): # False
                ...
                # print_rank_0(f'numel_prefetching: {numel_prefetching}', force=True)
