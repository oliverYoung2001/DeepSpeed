...
class DeepSpeedZeRoOffload(object):
    ...
    def __init__(
        ...
        self.forward_hooks = []
        self.backward_hooks = []
        self.setup_zero_stage3_hooks()  # [NOTE]: register hooks (pre/post forward/backward)
        print_rank_0(
    ...
    def _convert_to_zero_parameters(self, ds_config, module, mpu):
        ...
        #     print(f'param: {p}')
        #     print(f'is_zero_param(p): {is_zero_param(p)}')  # all True
        # print_rank_0(f'non_zero_params: {non_zero_params}', force=True) # [], why empty ???
        if non_zero_params:
    ...
    def setup_zero_stage3_hooks(self):
        ...
        def _end_of_forward_hook(module, *args):
            ...
            if not torch._C.is_grad_enabled():
                # print_rank_0(f'reset_step !!!', force=True) # Unreachable
    ...
    def pre_sub_module_forward_function(self, sub_module):
        ...
        # print_rank_0(f'sub_module: {sub_module}', force=True)
        # print_rank_0(f'sub_module.training: {sub_module.training}', force=True) # True

        param_coordinator = self.get_param_coordinator(training=sub_module.training)
        param_coordinator.trace_prologue(sub_module)
        if param_coordinator.is_record_trace():
            param_coordinator.record_module(sub_module)
        param_coordinator.fetch_sub_module(sub_module, forward=prev_grad_state) # [NOTE]: Allgather in here !!!
