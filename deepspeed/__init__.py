...
def print_model_param(model):
    ...
    # [NOTE]: print model size
    print(f'Total: {total_num}, Trainable: {trainable_num}, Total_size: {total_size} B, trainable_size: {trainable_size} B')
...
def initialize(args=None,
    ...
    if not isinstance(model, PipelineModule):   # pipe
        ...
        if config_class.hybrid_engine.enabled:  # False
            ...
            engine = DeepSpeedEngine(args=args,
                                     model=model,
                                     optimizer=optimizer,
                                     model_parameters=model_parameters, # None
                                     training_data=training_data,
                                     lr_scheduler=lr_scheduler,
                                     mpu=mpu,
                                     dist_init_required=dist_init_required,
    ...
    # if dist.get_rank() == 0:
    #     print(f'model after:')
    #     print_model_param(model)    # all 0
    # if dist.get_rank() == 0:
    #     print(f'engine:')
    #     print_model_param(engine)   # all 0
